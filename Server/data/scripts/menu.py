...
class Menu:
                        ...
                        for index, button in enumerate(buttons_stage_three):
                            if button.mouse_is_hovering():
                                ...
                                if index == 0:
                                    ...
                                    if self.connection_created(input_stage_three):
                                        # success
                                        return self.selected_mode
                                    else:
                                        # error
    ...
    def connection_created(self, input_stage_three):
        # function is called, when user presses the start button in "online"
        try:
            # host and port are read
            ...
            # server waits, until a client connects to the server
            self.server.listen()
            # server accepts the connection request
            self.connection, self.address = self.server.accept()
            
            # we must check, if both players selected the both gamemode
            # if the client doesn't awnser within 5 seconds, an error occurs
            self.connection.settimeout(5)
            # waiting to receive data
            data = self.connection.recv(4096).decode()
            # after receiving data, we send our gamemode to the client
            self.connection.send(bytes(self.selected_mode, "utf-8"))
            # throw an error visually, if the gammedoes differ
            ...
            # thrown, if an error occured in one of the connection steps
            self.server.close()
            return False
...
class Input:
    def __init__(self, position: tuple, size: tuple, label: str) -> None:
        ...
        # shown color when the input bar is inactive
        self.color_inactive = (50, 50, 50)
        # shown color when the input bar is active
        self.color_active = pygame.Color('black')
        # shown color
        self.color = self.color_inactive
        # variable which saved the status of the input bar
        self.active = False
        # saved included in the input bar
        self.text = ''
        # label
        self.label = label

    def render(self, root_surface):
        # rendering the input bar
        ...
        text_height = font.size(self.text)[1]
